...
from .views import (show_job_submitted, dynamic_input, dynamic_finished,
                    ...
                    behavior_results, behavior_output, edit_dynamic_behavioral,
...
urlpatterns = patterns('',
    ...
    # Redirect for a temporary page.
    url(r'^processing/(?P<pk>\d+)/', behavior_results, name='behavior_results'),
    url(r'^macro_processing/(?P<pk>\d+)/', elastic_results, name='elastic_results'),
    url(r'^behavior_results/(?P<pk>\d+)/', behavior_output, name='behavior_output'),
